...
Práce se zabývá specifiky německé gramatiky, které činí běžným n-gramovým modelům problémy. Nejprve popisuje statistické metody jazykového modelování a vysvětluje problematické jevy němčiny. Následně navrhuje vlastní varianty n-gramových jazykových modelů s cílem zlepšit korelaci ručně hodnocené plynulosti a automatického hodnocení - perplexity. Vlastní modely jsou trénovány jednak jako standardní n-gramové a jednak také metodou maximální entropie s n-gramovými rysy. Oba typy jsou vždy porovnány z hlediska úspěšnosti predikce a výpočetních nároků. Dále je navrhnuta množina vlastních rysů reprezentující počet gramatických chyb vybraných jevů. Úspěšnost se ověřuje na schopnosti predikovat ručně hodnocenou plynulost. Využito je modelů maximální entropie a vlastních modelů klasifikujících jen na základě mediánů hodnot rysů vypočtených z trénovacích dat.

Klíčová slova:
% 3 až 5 klíčových slov
jazykové modelování, němčina, n-gram, maximální entropie
...
\chapter{Úvod}
Už před několika desítkami let se zrodil nápad automatizovat překlad. Předpokládalo se, že to pro počítače nebude velký problém, a tak se hned po druhé světové válce začalo na této myšlence pracovat. Postupem času se ale ukázalo, že úkol to nebude vůbec lehký. První systémy byly velmi jednoduché, obsahovaly jen několik gramatických pravidel a malý slovník. Kvalita překladu proto byla špatná. Později se objevila myšlenka zašumněného kanálu, kde konečně přicházejí ke slovu jazykové modely. Překladový model z výchozího do cílového jazyka je nahrazen překladovým modelem obráceného směru tj. z jazyka cílového do jazyka zdrojového a jazykovým modelem cílového jazyka. Jazykový model přichází na řadu v době, kdy máme více možných hypotéz překladu a potřebujeme určit tu, která vypadá jako nejhezčí věta.

+ pokračování

%Na celou tuto problematiku se dá pohlížet z hlediska lingvistiky nebo z hlediska statistiky. 

%Myšlenka zašumněného kanálu, kdy se namísto 

%Později se začaly objevovat chytřejší systémy, ale i nové myšlenky, jakou je třeba rozpoznávání mluvené řeči. Ať už se jednalo o překlad nebo jinou úlohu, začalo být potřeba umět z více hypotéz

%Mimo strojový překlad se začaly objevovat i pokusy o rozpoznávání mluvené řeči. Oba 

%Až později se realizovala myšlenka frázového překladu, kdy se věta nejprve rozdělí na několik frází a teprve potom se dle naučených překladů těchto frází přeloží. Tím už ale začalo vznikat vždy velké množství překladových hypotéz a muselo se vymyslet, kterou z nich vybrat jako tu nejlepší. A právě zde se zrodil jazykový model, který tuto funkci v rámci překladu zajišťuje a snaží se z nabízených hypotéz vybrat tu nejlepší. Nejlepší ne ve smyslu přesnosti překladu, ale ve smyslu přirozeného slovosledu, gramatické správnosti a dalších zákonitostí daného cílového jazyka.

%Na strojový překlad se od dob jeho vzniku pohlíží dvěma způsoby - z hlediska lingvistiky a z hlediska statistiky. Statistické systémy dosahují obecně lepších výsledků, ačkoliv nevědí, jaké gramatické a morfologické zákonitosti jazyk má. Jsou vlastně založené jen na tom, jak pravděpodobný je výskyt daného úseku slov za sebou. Teoretické východisko pro omezení tohoto úseku na pevnou velikost formuloval už v roce 1912 
...
V porovnání je na tom maxentový model z hlediska perplexity opět mírně lépe. Stejně jako u předchozích modelů je na tom i proložená přímka, která klesá u standardního n-gramového modelu strměji (obrázek \ref{gr:porrsd}).
...
S rozšířeným slovním druhem už modely dopadly lépe, přesto perplexita s rostoucí plynulostí neklesá nijak výrazně, což dokazuje proložená přímka (obrázky \ref{gr:ngrcislo}, \ref{gr:maxcislo}).
...
Výsledky opět nejsou dobré (obrázky \ref{gr:ngpad}, \ref{gr:maxpad}) a jen potvrzují, že poskytnutí modelu značek pouze z jedné morfologické kategorie je nedostatečná informace.
